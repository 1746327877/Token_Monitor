...
test('migration preserves compact component order', () => {
  ...
  assert.deepEqual(
    ...
    ['quota-codex', 'quota-kimi', 'provider-bar', 'cost-line', 'balance-card', 'today-cost-card', 'cache-rate-card', 'model-bar', 'token-line', 'token-heatmap', 'opencode-stats-card']
  );
  assert.deepEqual(
    migrated.compact.items.map((item) => item.y),
    [0, 7, 14, 20, 26, 26, 26, 30, 36, 42, 52]
  );
  assert.deepEqual(
    migrated.compact.items.map((item) => item.x),
    [0, 0, 0, 0, 0, 4, 8, 0, 0, 0, 0]
  );
});
...
test('validation removes unknown and duplicate ids but restores missing records', () => {
  ...
  assert.deepEqual(
    ...
    ['balance-card', 'cache-rate-card', 'cost-line', 'model-bar', 'opencode-stats-card', 'provider-bar', 'quota-codex', 'quota-kimi', 'today-cost-card', 'token-heatmap', 'token-line']
  );
